...
import Disable2FactorController from "./Disable2FAController";

// API
import APIFetch from "../../../../util/APIFetch";
import Disable2FAAPI from "./Disable2FAAPI";

// Form
import FormHandler from "../../../../util/FormHandler";
import ValidateInput from "../../../../util/ValidateInput";
import Input from "../../../../components/Form/Input";
import Disable2FAForm from "./Disable2FAForm.json";

/**
 * User 2Factor Disable View
 *
 * Allows the user to disable 2Factor Authentication.
 *
 ...
 */
const Disable = ({ locale, languageData }) => {
    // Setup Initial State
    ...
    const [{ fetchResults }] = APIFetch({
        ...
        PATH_SEARCH: Disable2FAAPI.apiService.postDisable2FA.PATH_SEARCH,
        PATH_METHOD: Disable2FAAPI.apiService.postDisable2FA.PATH_METHOD,
        formData
    });
    ...
    /**
     * Handle User 2Factor Disable Actions
     *
     * Used to efficiently handle responses from the User 2Factor
     * Disable requests to either redirect the user or to display a
     * message to the user.
     *
    ...
    const handleResponseAction = (responseObj, data) => {
        // Handle appropriate actions with the User 2Factor Disable controller.
        const response = Disable2FactorController.handleAction(responseObj, data);
        ...
        const submitForm = async () => {
            if (!didCancel && submittingForm && !formErrors) {
                try {
                    ...
                    const disable2FactorResponse = await Disable2FactorController.handleResponse(response, languageData);

                    // Handle the result from the User 2Factor Disable request.
                    handleResponseAction(disable2FactorResponse, response.data);
                } catch (error) {
                    // Make sure we don't try to change state after re-render.
                    if (!didCancel) {
                        // Error will have a type if the result is from the User 2Factor DisableController.
                        if (get(error, "type")) {
     ...
     * will attempt to disable 2Factor for the user.
     *
    ...
    const handleFormSubmit = async e => {
        ...
        // API Service.
        Promise.all(map(Disable2FAForm, input => (
    ...
    // Render the User 2Factor Disable View.
    return (
        <>
            <Row>
                <Col className="mb-4">
                    { get(languageData, ["common", "user", "disable2Factor", "descrption"]) }
                </Col>
            </Row>
            <Form role="form" onSubmit={ handleFormSubmit } className="col-md-12">
                ...
                <FormGroup className="text-center">
                    <CustomButton text={ get(languageData, ["common", "disable2Factor"]) } className="my-3 bg-primary" type="submit" disabled={ submittingForm } />
                </FormGroup>
            </Form>
        </>
    );
...
// Export the User 2Factor Disable View.
